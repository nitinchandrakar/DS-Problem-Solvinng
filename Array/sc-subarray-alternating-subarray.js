...
const alterNativeSubArray = function (arr, B) {
  const N = arr.length;
  const tempLenth = 2*B+1;
  let ans = [];
  for (let i = 0; i < N; i++) {
    let temp = -1;
    let isCorrectArray = true;
    for (let j = i, k=0; k < tempLenth; j++, k++) {
        if(k==0){
            temp = arr[j];
        }else if(k % 2 == 0){
            ...
            if(!((temp==0 && arr[j]==1) || (temp==1 && arr[j]==0))){
                isCorrectArray = false;
            }
        }
    }
    if(isCorrectArray){
        let center = i+(tempLenth - 1)/2
        ans.push(center);
    }
  }
...
console.log(alterNativeSubArray( [1, 0, 1, 0, 1],1))
console.log(alterNativeSubArray( [0, 0, 0, 1, 1, 0, 1],0))